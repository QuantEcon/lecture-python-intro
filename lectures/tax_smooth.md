...
(a) Compute the optimal constant tax rate $T_0$ for the baseline expenditure
    sequence (without the spike) and for the sequence with the spike.  Print the
    difference $\Delta T_0$.
...
# Baseline: constant expenditures
G_base = np.ones(S+1)
T_base, _, _ = compute_optimal(tax_model, B0, G_base)

# Shocked: one-time spike at t*
t_star = 20
W0 = 1.0
G_shock = G_base.copy()
G_shock[t_star] += W0
...
print(f'Numerical ΔT₀:           {delta_T0_numerical:.10f}')
print(f'Annuity-value formula:   {delta_T0_formula:.10f}')
print(f'Match: {np.isclose(delta_T0_numerical, delta_T0_formula)}')
```

The spike raises $h_0$ (present value of expenditures) by exactly $W_0 R^{-t^*}$.
Since the optimal flat tax is the annuity value of $(B_0 + h_0)$, the increase in
$T_0$ is the annuity value of the extra present-value cost.
...
(a) On one set of axes, plot the four expenditure sequences.  On a second set,
    plot the corresponding debt paths $B_t$.

(b) Which $\lambda$ requires the highest flat tax?  Explain in terms of the
    present-value formula for $h_0$.
```
...
Faster spending growth ($\lambda$ higher) raises the present value $h_0$, because
early, heavily-discounted expenditures are replaced by larger later ones.  A higher
$h_0$ translates directly into a higher required flat tax.
...
Use the default expenditure sequence and $B_0 = 2$.  Evaluate welfare for the
...
Print a table showing each path's welfare and its gap relative to the optimum.
```
...
print(f'{"ξ₁":>6} | {"ϕ":>6} | {"welfare":>12} | {"gap":>14}')
print('-' * 46)
for ξ1, ϕ in params:
...
Every variation yields strictly lower welfare (more negative gap) than the flat
path, confirming that the constant tax schedule is the global welfare maximiser
...
print(f'Numerical slope of T₀ w.r.t. B₀: {slope:.8f}')
...
The coefficient on $B_0$ is exactly the annuity factor, confirming the linear
relationship.  Higher initial debt forces the government to collect more tax in
every period to satisfy its intertemporal budget constraint.
